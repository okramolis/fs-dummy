...
Dummy.prototype.ensureFile = function(callback) {
  // At first make sure the dummy root directory exists, then
  // overwrite current file if it exists or create the new one.
  async.series([
...
Dummy.prototype.ensureFolder = function(callback) {
  // At first make sure the dummy root directory exists,
  // then remove current folder and finally ensure the folder.
  async.series([
    this._verifyRootStat.bind(this),
    this._removeFolder.bind(this),
    this._createFolder.bind(this)
  ], callback);
}
...
Dummy.prototype.cleanupFile = function(callback) {
  // At first make sure the dummy root directory exists,
  // then make sure the dummy file does not exist.
  async.series([
    this._verifyRootStat.bind(this),
    this._removeFile.bind(this)
  ], callback);
}

Dummy.prototype.cleanupFolder = function(callback) {
  // At first make sure the dummy root directory exists,
  // then make sure the dummy folder does not exist.
  async.series([
    this._verifyRootStat.bind(this),
    this._removeFolder.bind(this)
  ], callback);
...
// Makes sure the file does not exist in the root test directory.
Dummy.prototype._removeFile = function(callback) {
  fs.remove(path.join(this._root, this._file), callback);
}

// Makes sure the folder does not exist in the root test directory.
Dummy.prototype._removeFolder = function(callback) {
  fs.remove(path.join(this._root, this._folder), callback);
}
